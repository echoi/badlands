...
contains
  ...
  subroutine getSPM_hdf5topography
    ...
    integer(ESMF_KIND_I4)::id,i,rank,rpet,localNd,p

    real(ESMF_KIND_R8),dimension(:),allocatable::nodes,nID
        ...
        do p=1,localNd
            ...
            if(abs(nodes(id)-tcoordX(i))>0.01.or.abs(nodes(id+1)-tcoordY(i))>0.01)then
                ...
                call ESMF_LogSetError(rcToCheck=ESMF_RC_FILE_OPEN, &
                    msg="Failed during restart when building coordinate elevation value", &
  ...
  ! =====================================================================================
  subroutine getSPM_hdf5strata

    logical::compression,simple

    integer(ESMF_KIND_I4)::id,i,k,rank,rpet,localNd,p

    real(ESMF_KIND_R8),dimension(:),allocatable::zstrat,nID,facTyp
    ...
    do rpet=0,restartPet-1
        ...
        layerID=dims(1)
        localNd=dims(2)
        allocate(zstrat(layerID*localNd))
        if(rpet==0)then
            if(allocated(stratalZ)) deallocate(stratalZ)
            layerNb=int((time_end-time_start)/layer_interval)+layerID
            allocate(stratalZ(layerNb,dnodes))
            stratalZ=0.
        endif
        call h5dread_f(dset_id,h5t_native_double,zstrat,dims,rc)
        id=0
        do p=1,localNd
            i=int(nID(p))
            do k=1,layerID
                id=id+1 
                stratalZ(k,i)=zstrat(id)
            enddo
        enddo
        ...
        layerID=dims(1)
        localNd=dims(2)
        allocate(facTyp(layerID*localNd))
        if(rpet==0)then
            if(allocated(stratalFacies)) deallocate(stratalFacies)
            allocate(stratalFacies(layerNb,dnodes))
            stratalFacies=0.
        endif
        call h5dread_f(dset_id,h5t_native_double,facTyp,dims,rc)
        id=0
        do p=1,localNd
            i=int(nID(p))
            do k=1,layerID
                id=id+1 
                stratalFacies(k,i)=facTyp(id)
            enddo
        enddo
        ...
        deallocate(zstrat,nID,facTyp)
    enddo

    return

  end subroutine getSPM_hdf5strata
  ! =====================================================================================
